...
export default function Form({ entries }) {
	...
	const handleNormalClick = (checked: boolean, id: string, index: number) => {
		setSelectedInputs((prevInputs) => (checked ? [...prevInputs, id] : prevInputs.filter((inputId) => inputId !== id)));
		setStartShiftClickIndex(index);
	};

	const handleCommandClick = (id: string) => {
		setSelectedInputs((prevInputs) =>
			prevInputs.includes(id) ? prevInputs.filter((inputId) => inputId !== id) : [...prevInputs, id],
		);
	};
	...
	const handleShiftClick = (index: number, checked: boolean) => {
		...
		setSelectedInputs((prevInputs) => {
			const newSelection = entries.slice(startIndex, endIndex + 1).map((item) => item.id);

			if (checked) {
				const combinedSelection = Array.from(new Set([...prevInputs, ...newSelection]));
	...
	const handleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>, id: string, index: number) => {
		if (event.key === "Enter") {
			...
			{entries.map((entry, index) => (
				<GuestbookEntry key={entry.guestbook.id} entry={entry}>
					<input
						...
						onChange={(e) => handleCheck(e.target.checked, entry.guestbook.id, index)}
	...
	return (
		<div className="flex flex-col space-y-1 mb-4">
			<div className="w-full text-sm break-words items-center flex">
				{children}
				<span className="text-neutral-600 dark:text-neutral-400 mr-1 border-neutral-100">{entry.user.name}:</span>
