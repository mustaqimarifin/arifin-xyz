...
export const generateStaticParams = async () => projectParam.map((p) => ({ slug: p.slug }));
		...
		<section>
			...
			<div className="flex justify-between items-center mt-2 mb-8 text-sm max-w-[650px]">
				<Suspense fallback={<p className="h-5" />}>
					<p className="text-sm text-neutral-600 dark:text-neutral-400">{formatDateXtra(post.year)}</p>
				</Suspense>
				<Suspense fallback={<p className="h-5" />}>
