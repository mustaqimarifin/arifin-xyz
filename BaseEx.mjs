...
class BytesOutput {
  static get typeList() {
    return ['buffer', 'bytes', 'uint8', 'view']
  }
  ...
  static toBytes(input, settings) {
    ...
    // Array
      ...
      for (const elem of input) {
        collection.push(...SmartInput.toBytes(elem, settings)[0])
      }
...
class SmartOutput {
  ...
  static makeTypedArrayBuffer(
    ...
    const delta =
      (bytesPerElem - (Uint8ArrayOut.byteLength % bytesPerElem)) % bytesPerElem
  ...
  static makeTypedArray(inArray, type, littleEndian, negative) {
    ...
    if (type === 'int16' || type === 'uint16') {
      const buffer = SmartOutput.makeTypedArrayBuffer(
        inArray,
        2,
        littleEndian,
        negative
      ...
      outArray =
        type === 'int16' ? new Int16Array(buffer) : new Uint16Array(buffer)
  ...
  static compile(Uint8ArrayOut, type, littleEndian = false, negative = false) {
    ...
    if (negative) {
      let n
      if (type.match(/^float/)) {
        n = -SmartOutput.compile(Uint8ArrayOut, 'float_n', littleEndian)
      } else {
        n = -SmartOutput.compile(Uint8ArrayOut, 'uint_n', littleEndian)
      }
      if (type === 'float_n') {
        return n
      }
      Uint8ArrayOut = SmartInput.toBytes(n, {
        littleEndian,
        numberMode: false,
        signed: false,
      })[0]
    ...
    if (type === 'buffer') {
      ...
      if (Uint8ArrayOut.length === 1) {
        const uint16Buffer = SmartOutput.makeTypedArrayBuffer(
          Uint8ArrayOut,
          2,
          littleEndian,
          negative
      ...
      if (Uint8ArrayOut.length <= 4) {
        ...
        if (Uint8ArrayOut.length === 4) {
          ...
          array = SmartOutput.makeTypedArray(
            Uint8ArrayOut,
            'float32',
            false,
            negative
        ...
        throw new RangeError(
          'The provided input is to complex to be converted into a floating point.'
        )
      }
    } else if (type === 'number') {
      if (Uint8ArrayOut.length !== 8) {
      ...
      compiled = SmartOutput.makeTypedArray(
        Uint8ArrayOut,
        type,
        littleEndian,
        negative
...
class Utils {
  ...
  setIOHandlers(
    ...
    outputHandler = DEFAULT_OUTPUT_HANDLER
  ) {
    ...
    this.root.addCharset = (name, _charset, _padChars = [], info = true) => {
      const normalize = (typeName, set, setLen) => {
        if (setLen === 0 && set.length) {
          console.warn(
            `This converter has no ${typeName}. The following argument was ignored:\n'${set}'`
          )
          return []
        }
        ...
        if (Array.isArray(set)) {
          ...
          throw new TypeError(
            `The ${typeName} must be one of the types:\n'str', 'set', 'array'."`
          )
        }
        ...
        if (inputLen !== setLen) {
          throw new Error(
            `Your ${typeName} has a length of ${inputLen}. The converter requires a length of ${setLen}.`
          )
        }
        const charAmounts = {}
        _charset = [..._charset]
        ...
        const rChars = Object.keys(charAmounts).filter(
          (c) => charAmounts[c] > 1
        )
        throw new Error(
          `You have repetitive char(s) [ ${rChars.join(' | ')} ] in your ${typeName}. Make sure each character is unique.\n${infoStr}`
        )
      }
      ...
      const padChars = normalize(
        'padding set',
        _padChars,
        this.root.padCharAmount
      )
  ...
  #invalidArgument(arg, versions, outputTypes, initial) {
    const loopConverterArgs = () =>
      Object.keys(this.converterArgs)
        .map((key) =>
          this.converterArgs[key].map((keyword) => `'${keyword}'`).join(' and ')
        )
        .join('\n   - ')
  ...
  validateArgs(args, initial = false) {
    ...
    if (initial) {
      if (extractArg('bytes_only')) {
        ...
        const inHandler = extractArg('bytes_in')
          ? BytesInput
          : DEFAULT_INPUT_HANDLER
        const outHandler = extractArg('bytes_out')
          ? BytesOutput
    ...
    if (extractArg('decimal')) {
      if (!this.root.hasDecimalMode) {
        throw TypeError(
          `Argument 'decimal' is only allowed for converters with a non-integer base.`
        )
      }
        ...
        for (const param in extraArgList) {
          if (extraArgList[param].includes(arg)) {
            ...
            if (this.root.isMutable[param]) {
              ...
              throw TypeError(
                `Argument '${arg}' is not allowed for this type of converter.`
              )
            }
          }
        }
  ...
  encode(inputBytes, charset, littleEndian = false, replacer = null) {
    ...
    for (let i = 0, l = byteArray.length; i < l; i += bs) {
      ...
      while (q >= this.radix) {
        ...
        bXarray.unshift(Number.parseInt(r, 10))
      }

      // Append the remaining quotient to the array
      bXarray.unshift(Number.parseInt(q, 10))
      ...
      let frame = ''
      bXarray.forEach((charIndex) => (frame = frame.concat(charset[charIndex])))
  ...
  decode(
    ...
    for (let i = 0, l = byteArray.length; i < l; i += bs) {
      ...
      while (q >= 256) {
        ...
        subArray256.unshift(Number.parseInt(r, 10))
      }

      // Append the remaining quotient to the array
      subArray256.unshift(Number.parseInt(q, 10))
...
class BaseTemplate {
  ...
  encode(input, replacerFN, postEncodeFN, ...args) {
    ...
    // handle input
    let [inputBytes, negative, type] = this.utils.inputHandler.toBytes(
      input,
      settings
    )
    ...
    let [output, zeroPadding] = this.converter.encode(
      inputBytes,
...
class Base1 extends BaseTemplate {
  ...
  encode(input, ...args) {
    ...
    let base10 = this.converter.encode(
      inputBytes,
      null,
      settings.littleEndian
    )[0]
    ...
    if (n > 16777216) {
      console.warn(
        'The string length is really long. The JavaScript engine may have memory issues generating the output string.'
      )
    }
  ...
  decode(input, ...args) {
    ...
    // Run the decoder
    const output = this.converter.decode(
      input,
      [...'0123456789'],
      [],
      '',
  ...
  decode(input, ...args) {
    ...
    const normalizeInput = ({ input: normInput, settings }) => {
      ...
      if (!settings.integrity) {
        normInput = normInput.toLowerCase().replace(/[^0-9a-f]/g, '')
      }
...
class Base64 extends BaseTemplate {
  ...
  constructor(...args) {
    ...
    this.charsets.urlsafe = this.charsets.default
      .slice(0, -2)
      .concat(['-', '_'])
  ...
  decode(input, ...args) {
    ...
    const format = ({ input, settings }) => {
      ...
      for (const line of lines) {
        ...
        // fix probably missing spaces for original charset
        if (lArray.length !== 60 && settings.version === 'original') {
...
class LEB128 extends BaseTemplate {
  ...
  encode(input, ...args) {
    ...
    const [inputBytes, negative] = this.utils.inputHandler.toBytes(
      input,
      settings
    )
  ...
  decode(input, ...args) {
    ...
    if (settings.version === 'hex') {
      input = this.hexlify.decode(
        this.utils.normalizeInput(input).toLowerCase(),
        [...'0123456789abcdef'],
        [],
        settings.integrity,
        false
    ...
    const applyPadding = ({ output, settings, zeroPadding }) => {
      ...
      if (zeroPadding > 1) {
        ...
        if (settings.padding) {
          ...
          const padArr = new Array(padLen).fill(
            this.padChars[settings.version].at(-1)
          )
  ...
  decode(input, ...args) {
    ...
    const decode = (input) => {
      if (versionKey !== null) {
        ...
        version = versionKey === 3 ? settings.version : `emojis_v${versionKey}`
      }
  ...
  #preDecode(input, versionKey, integrity) {
      ...
      if (char in this.#revEmojiVersion) {
        ...
        if (charVersion !== 3) {
          if (versionKey === 3) {
            ...
            throw new TypeError(
              `Emojis from different ecoji versions seen : ${char} from emojis_v${charVersion}`
            )
          }
        }
        ...
        if (integrity) {
          ...
          if (padding) {
            ...
            if (padding === 'fill') {
              if (relIndex === 0) {
                throw new TypeError(
                  `Padding unexpectedly seen in first position ${char}`
                )
              }
            } else if (relIndex !== 3) {
              throw new TypeError(
                `Last padding seen in unexpected position ${char}`
              )
            }
          } else if (sawPadding) {
    ...
    if (integrity && inArray.length % 4) {
      if (
        ...
        throw new TypeError(
          'Unexpected end of data, input data size not multiple of 4'
        )
      }
    }
        ...
        if (z > -1) {
          if (i + 1 !== inArray.length) {
            throw new DecodingError(
              null,
              `Secondary character found before end of input, index: ${i}`
            )
          }
...
class SimpleBase extends BaseTemplate {
  ...
  constructor(radix, ...args) {
    ...
    if (!radix || !Number.isInteger(radix) || radix < 2 || radix > 62) {
      throw new RangeError(
        'Radix argument must be provided and has to be an integer between 2 and 62.'
      )
    }
    ...
    // list of allowed/disallowed args to change
    ;(this.isMutable.littleEndian = true), (this.isMutable.upper = radix <= 36)
...
function _Big_() {
  function Big(n) {
    if (!(this instanceof Big)) return void 0 === n ? _Big_() : new Big(n)
    if (n instanceof Big) (this.s = n.s), (this.e = n.e), (this.c = n.c.slice())
    else {
      if ('string' !== typeof n) {
        if (!0 === Big.strict && 'bigint' !== typeof n)
...
function parse(x, n) {
  ...
  for (
    x.s = '-' === n.charAt(0) ? ((n = n.slice(1)), -1) : 1,
      ...
      (i = n.search(/e/i)) > 0
        ? (e < 0 && (e = i), (e += +n.slice(i + 1)), (n = n.substring(0, i)))
        : e < 0 && (e = n.length),
      nl = n.length,
      i = 0;
    i < nl && '0' === n.charAt(i);
  ...
  if (sd < 1)
    (more =
      ...
      (0 === sd &&
        ((1 === rm && xc[0] >= 5) ||
          (2 === rm &&
            (xc[0] > 5 || (5 === xc[0] && (more || void 0 !== xc[1]))))))),
      (xc.length = 1),
      more ? ((x.e = x.e - sd + 1), (xc[0] = 1)) : (xc[0] = x.e = 0)
...
;(P.abs = function () {
  ...
  (P.cmp = function (y) {
    ...
    for (j = (k = xc.length) < (l = yc.length) ? k : l, i = -1; ++i < j; )
  ...
  (P.minus = P.sub =
    function (y) {
      ...
      let ye = y.e
      if (!xc[0] || !yc[0])
        return yc[0] ? (y.s = -b) : xc[0] ? (y = new Big(this)) : (y.s = 1), y
      if ((a = xe - ye)) {
        for (
  ...
  (P.plus = P.add =
    function (y) {
      ...
      let t
      let Big = this.constructor
      if (((y = new Big(y)), this.s !== y.s)) return (y.s = -y.s), this.minus(y)
      let xe = this.e
      let xc = this.c
      let ye = y.e
      let yc = y.c
      if (!xc[0] || !yc[0])
        return yc[0] || (xc[0] ? (y = new Big(this)) : (y.s = this.s)), y
      if (((xc = xc.slice()), (e = xe - ye))) {
        for (
          e > 0 ? ((ye = xe), (t = yc)) : ((e = -e), (t = xc)), t.reverse();
          e--;
      ...
      for (k && (xc.unshift(k), ++ye), e = xc.length; 0 === xc[--e]; ) xc.pop()
  ...
  (P.toFixed = function (dp, rm) {
    ...
    if (void 0 !== dp) {
      ...
      for (
        x = round(new x.constructor(x), dp + x.e + 1, rm), dp = dp + x.e + 1;
        x.c.length < dp;

      )
        x.c.push(0)
    }
    return stringify(x, !1, !!n)
  }),
  (P[Symbol.for('nodejs.util.inspect.custom')] =
    ...
    P.toString =
      function () {
        ...
        return stringify(
          this,
          this.e <= Big.NE || this.e >= Big.PE,
          !!this.c[0]
        )
      }),
  (P.toNumber = function () {
    let n = Number(stringify(this, !0, !0))
    if (!0 === this.constructor.strict && !this.eq(n.toString()))
...
class BasePhi extends BaseTemplate {
  ...
  encode(input, ...args) {
    ...
    if (settings.decimalMode) {
      if (Number.isFinite(input)) {
        ...
        throw new TypeError(
          "When running the converter in decimal-mode, only input of type 'Number' is allowed."
        )
      }
    }
    ...
    // of the input to base 10.
      ...
      ;[inputBytes, negative] = this.utils.inputHandler.toBytes(input, settings)
      n = Big(this.b10.encode(inputBytes, null, settings.littleEndian)[0])
    }
    ...
    const reduceN = (cur, prev, exp) => {
      ...
      while (cur.gt(n)) {
        ...
        if (cur.lte(0)) {
          console.warn(
            'Could not find an exact base-phi representation. Value is approximated.'
          )
          return
        }
        exp--
  ...
  decode(input, ...args) {
    ...
    if (settings.integrity && inputSplit.length > 2) {
      throw new DecodingError(
        null,
        'There are multiple decimal points in the input.'
      )
    }
   ...
   */
  #approxNull(n) {
    return !n.round(50).abs().toNumber()
  }
